...
local DrawingFunctions = {
    NewLine = function(Thickness)
        ...
        Line.Visible = true
        Line.Color = Color3.fromRGB(255, 192, 203)
        Line.Thickness = 1
        Line.Transparency = 1
        Line.ZIndex = 2
...
local IsolatedFunctions = {
    TargetStrafe = function(IsAutoJump, IsControllable, GetSpeed, GetDistance, GetEnemy)
        if IsControllable then
            if IsolatedStorage.Side == "Right" then
                IsolatedStorage.GetStrafeAngle = IsolatedStorage.GetStrafeAngle + GetSpeed
            else
                IsolatedStorage.GetStrafeAngle = IsolatedStorage.GetStrafeAngle - GetSpeed
            end
        else
            if IsolatedStorage.GetStrafeAngle > 360 then
                IsolatedStorage.GetStrafeAngle = 0
            else
                IsolatedStorage.GetStrafeAngle = IsolatedStorage.GetStrafeAngle + GetSpeed
            end
        end
        LocalPlayer.Character.HumanoidRootPart.CFrame = GetEnemy.CFrame * CFrame.Angles(0, math.rad(IsolatedStorage.GetStrafeAngle), 0) * CFrame.new(0, 0, GetDistance)
    end,
    NearestDistance = function()
        ...
        for i, v in next, Players:GetPlayers() do
            if v ~= LocalPlayer and IsNetwork(LocalPlayer) and IsNetwork(v) then
                ...
                if (not IsolatedSettings.Target.NearestDistanceBehindCheck or RootVisible) then
    ...
    Notify = function(Desc, Delay, Type)
        ...
        NovoIntentNotification.BackgroundColor3 = Color3.fromRGB(255, 192, 203)
        ...
        Name.Text = "Isolated Notification"
...
local IsolatedMiscFunctions = {
        ...
        if IsNetwork(LocalPlayer) then
            ...
            for i, v in pairs(Parts) do
                if v.Transparency >= 0.3 then
                    IsolatedStorage.Instance[#IsolatedStorage.Instance + 1] = v
                end
    ...
    NearestType = function(Type)
        if Type == "Distance" then
            return IsolatedFunctions.NearestDistance()
        elseif Type == "Mouse" then
            return IsolatedFunctions.NearestMouse()
        end
    end
}
...
Title.Text = "Isolated | Da Hood | 60 fps | 30 ms | 00:00:00"
...
local Window = IsolatedLibrary.new("Isolated Internal", "v2.0.0", nil)
...
TargetFolder.Toggle("Use Distance Nearest", function(State)
    if State then
        IsolatedSettings.Target.UseMouseNearest = "Distance"
    else
        IsolatedSettings.Target.UseMouseNearest = "Mouse"
    end
end)

TargetFolder.Toggle("Nearest Distance Behind Check", function(State)
    IsolatedSettings.Target.NearestDistanceBehindCheck = State
end)

TargetFolder.Toggle("Wall Check", function(State)
    IsolatedSettings.Target.WallCheck = State
end)

TargetFolder.Toggle("Use Old WallCheck", function(State)
    IsolatedSettings.Target.UseOldWallCheck = State
    IsolatedSettings.Target.UseNewWallCheck = not State
end)

TargetFolder.Toggle("Grabbed Check", function(State)
    IsolatedSettings.Target.Grabbed = State
end)

TargetFolder.Toggle("Knocked Check", function(State)
    IsolatedSettings.Target.Knocked = State
end)

local TargetStrafeModule = CombatTab.Cheat("TargetStrafe", "Rotate Around Target & Break Target Lock Velocity.", function(State)
    IsolatedSettings.TargetStrafe.Enabled = State
end)

TargetStrafeModule.Toggle("Auto Jump", function(State)
    IsolatedSettings.TargetStrafe.AutoJump = State
end)

TargetStrafeModule.Toggle("Controllable", function(State)
    IsolatedSettings.TargetStrafe.Controllable = State
end)

if not Optimized then
    TargetStrafeModule.Toggle("Show Rotation Range", function(State)
        IsolatedSettings.TargetStrafe.ShowRotationRange = State
    end)
end

TargetStrafeModule.Toggle("Inf Range", function(State)
    IsolatedSettings.TargetStrafe.InfRange = State
    if IsolatedSettings.TargetStrafe.InfRange then
        IsolatedSettings.TargetStrafe.Range = math.huge
    else
        IsolatedSettings.TargetStrafe.Range = IsolatedStorage.StoredRange
    end
end)

TargetStrafeModule.Toggle("Hold Space", function(State)
    IsolatedSettings.TargetStrafe.HoldSpace = State
end)

TargetStrafeModule.Slider("Distance", {Precise = true, Default = 5, Min = 2, Max = 25}, function(State)
    IsolatedSettings.TargetStrafe.Distance = State
end)

TargetStrafeModule.Slider("Range", {Precise = true, Default = 15, Min = 2, Max = 50}, function(State)
    IsolatedStorage.StoredRange = IsolatedSettings.TargetStrafe.Range
    if IsolatedSettings.TargetStrafe.InfRange then
        IsolatedSettings.TargetStrafe.Range = math.huge
    else
        IsolatedSettings.TargetStrafe.Range = State
    end
end)

TargetStrafeModule.Slider("Speed", {Precise = true, Default = 4, Min = 2, Max = 10}, function(State)
    IsolatedSettings.TargetStrafe.Speed = State
end)
...
FlyModule.Toggle("Move Only", function(State)
    IsolatedSettings.Fly.MoveOnly = State
    if State == false and IsolatedSettings.Loaded then
        if IsNetwork(LocalPlayer) then
            LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.new(0, 24, 0)
        end
...
DisablerFolder.Toggle("CFrame", function(State)
    IsolatedSettings.Disabler.CFrameDetection = State
end)

DisablerFolder.Toggle("Ban", function(State)
    IsolatedSettings.Disabler.Ban = State
end)

DisablerFolder.Toggle("Crash", function(State)
    IsolatedSettings.Disabler.Crash = State
end)

MiscTab.Cheat("No Rotation", "Prevent LocalPlayer From Rotating By Shiftlock. & Auto Rotating", function(State)
    IsolatedSettings.NoRotation.Enabled = State
end)

local TimeTick
TimeTick = hookfunction(wait, function(JumpCooldown)
    if JumpCooldown == 1.5 and (IsolatedSettings.Speed.BHop and IsolatedSettings.Speed.Enabled) or IsolatedSettings.NoJumpCd.Enabled then 
        return TimeTick()
    end
...
UIs.InputBegan:Connect(function(Input, GameProcessed)
    ...
    if Input.KeyCode == Enum.KeyCode.Q then
        IsolatedStorage.Macro = true
    end
    if Input.KeyCode == Enum.KeyCode.RightShift then
        Window:Toggle()
    end
    if Input.KeyCode == Enum.KeyCode.A then
        IsolatedStorage.Side = "Left"
    end
    if Input.KeyCode == Enum.KeyCode.D then
        IsolatedStorage.Side = "Right"
    end
    if Input.KeyCode == Enum.KeyCode.Space then
        IsolatedStorage.HeldSpace = true
    end
    if Input.UserInputType == Enum.UserInputType.MouseButton2 then
        IsolatedStorage.RotatingCamera = true
    end
end)
...
UIs.InputEnded:Connect(function(Input, GameProcessed)
    ...
    if Input.KeyCode == Enum.KeyCode.Space then
        IsolatedStorage.HeldSpace = false
    end
    if Input.KeyCode == Enum.KeyCode.Q then
        IsolatedStorage.Macro = false
    end
    if Input.UserInputType == Enum.UserInputType.MouseButton2 then
        IsolatedStorage.RotatingCamera = false
    end
end)

RunService.Heartbeat:Connect(function()
    if IsolatedSettings.Watermark.Enabled then
        IsolatedStorage.FPSBeat = IsolatedStorage.FPSBeat + 1
    end

    if IsolatedSettings.MotionBlur.Enabled then
        if not Blur and CurrentCamera:FindFirstChild("Blur") then Instance.new("Blur", CurrentCamera).Size = 0 end
        Blur.Size = math.abs((CurrentCamera.CFrame.lookVector - Last).Magnitude) * IsolatedSettings.MotionBlur.BlurAmount
        Last = CurrentCamera.CFrame.lookVector
    end

    if IsolatedSettings.SilentAim.DrawingCircle then
        DrawingCircle.Radius = IsolatedSettings.SilentAim.DrawingCircleSize
        DrawingCircle.Position = Vector2.new(Mouse.X, Mouse.Y + GuiInset.Y)
        DrawingCircle.Visible = true

        OutlineDrawingCircle.Radius = IsolatedSettings.SilentAim.DrawingCircleSize
    ...
    if IsNetwork(LocalPlayer) then
        if IsolatedSettings.SpeedGlitch.Enabled and IsolatedStorage.Macro then
            if LocalPlayer.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Freefall then
                LocalPlayer.Character.HumanoidRootPart.Velocity = CurrentCamera.CFrame.lookVector * IsolatedSettings.SpeedGlitch.SpeedMultiplier
                if IsolatedStorage.Angle == 1 then
                    ...
                    IsolatedStorage.Angle = 0
                else
                    LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame * CFrame.Angles(0, math.rad(-45), 0)
                    IsolatedStorage.Angle = 1
                end
            end
        end

        if IsolatedSettings.AutoHeal.Enabled then
            if LocalPlayer.Character.Humanoid.Health < IsolatedSettings.AutoHeal.HealthBelow then
                for i, v in pairs(LocalPlayer.Character:GetChildren()) do
                    if v:FindFirstChild("Eat") or v:FindFirstChild("Drink") then
                        v:Activate()
                    else
                        IsolatedStorage.Equipable = true
                    end
                end
            end
            if IsolatedStorage.Equipable then
                pcall(function()
                    for i3, v3 in pairs(LocalPlayer.Backpack:GetChildren()) do
                        if v3:FindFirstChild("Eat") or v3:FindFirstChild("Drink") then
                            ...
                            IsolatedStorage.Equipable = false
                        end
                    end
                end)
            end
        ...
        if IsolatedSettings.Speed.Enabled and not IsolatedSettings.Fly.Enabled then
            if LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 then
                if IsolatedSettings.Speed.Motion then
                    LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection * IsolatedSettings.Speed.Amount / 1.5)
                end
                if IsolatedSettings.Speed.BHop and LocalPlayer.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Freefall then
                    LocalPlayer.Character.Humanoid:ChangeState("Jumping")
                end
            end
        end
        if IsolatedSettings.SilentAim.Enabled then
            if IsolatedSettings.SilentAim.SilentRotation then
                local GetEnemy, GetEnemyPos = IsolatedMiscFunctions.NearestType(IsolatedSettings.Target.UseMouseNearest)
                if IsNetwork(GetEnemy) then
                    ...
                    if GetEnemy.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Freefall or GetEnemy.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Jumping and (not IsolatedSettings.Target.Knocked or not IsolatedFunctions.Knocked(NearestTarget)) then
        ...
        if IsolatedSettings.TargetStrafe.Enabled then
            local GetEnemy, GetEnemyPos = IsolatedMiscFunctions.NearestType(IsolatedSettings.Target.UseMouseNearest)
            if GetEnemy and GetEnemy.Character and IsolatedSettings.TargetStrafe.Range > (LocalPlayer.Character.HumanoidRootPart.Position - GetEnemy.Character.HumanoidRootPart.Position).Magnitude and (not IsolatedSettings.Target.Knocked or not IsolatedFunctions.Knocked(NearestTarget)) and (not IsolatedSettings.TargetStrafe.HoldSpace or IsolatedStorage.HeldSpace) then
                if IsolatedSettings.TargetStrafe.AutoJump and LocalPlayer.Character.Humanoid:GetState() ~= Enum.HumanoidStateType.Freefall then
                    LocalPlayer.Character.Humanoid:ChangeState("Jumping")
                end
                IsolatedFunctions.TargetStrafe(IsolatedSettings.TargetStrafe.AutoJump, IsolatedSettings.TargetStrafe.Controllable, IsolatedSettings.TargetStrafe.Speed, IsolatedSettings.TargetStrafe.Distance, GetEnemy.Character.HumanoidRootPart)
            end
        end

        if not IsolatedStorage.Macro then
            if IsolatedSettings.NoRotation.Enabled then
        ...
        if IsolatedSettings.Fly.Enabled and (not IsolatedSettings.Fly.MoveOnly or LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0) then
            if IsolatedSettings.Fly.Normal then
                local AngleX, AngleY, AngleZ = LocalPlayer.Character.HumanoidRootPart.CFrame:ToEulerAnglesYXZ()
                LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(LocalPlayer.Character.HumanoidRootPart.CFrame.X, IsolatedSettings.Fly.Height + 24, LocalPlayer.Character.HumanoidRootPart.CFrame.Z) * CFrame.Angles(AngleX, AngleY, AngleZ)
                LocalPlayer.Character.Humanoid:ChangeState("Freefall")
                LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection * IsolatedSettings.Fly.Amount / 1.5)
            end
        end

        if IsolatedSettings.Strafe.Enabled then
            if LocalPlayer.Character.Humanoid.MoveDirection.Magnitude > 0 and LocalPlayer.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall and not IsolatedSettings.Speed.Enabled then
                LocalPlayer.Character:TranslateBy(LocalPlayer.Character.Humanoid.MoveDirection / 2.1)
            end
        end

        if not Optimized then
            if IsolatedSettings.TargetStrafe.ShowRotationRange then
                local GetEnemy, GetEnemyPos = IsolatedMiscFunctions.NearestType(IsolatedSettings.Target.UseMouseNearest)
                if GetEnemy then
                    local RootPos = GetEnemy.Character.HumanoidRootPart.Position
                    local Vertices = IsolatedFunctions.GetVertices(Vector3.new(RootPos.X, RootPos.Y - 2, RootPos.Z), IsolatedSettings.TargetStrafe.Distance)
                    
                    local PointA, z, AVisible = IsolatedFunctions.ToScreen(Vertices.Top)
                    local PointB, z, BVisible = IsolatedFunctions.ToScreen(Vertices.Corner1)
                    local PointC, z, CVisible = IsolatedFunctions.ToScreen(Vertices.Right)
                    local PointD, z, DVisible = IsolatedFunctions.ToScreen(Vertices.Corner2)
                    local PointE, z, EVisible = IsolatedFunctions.ToScreen(Vertices.Bottom)
                    local PointF, z, FVisible = IsolatedFunctions.ToScreen(Vertices.Corner3)
                    local PointG, z, GVisible = IsolatedFunctions.ToScreen(Vertices.Left)
                    local PointH, z, HVisible = IsolatedFunctions.ToScreen(Vertices.Corner4)
...
Workspace.Ignored.ChildAdded:Connect(function(newChild)
    wait()
    if newChild.Name == "SnowBlock" and IsolatedSettings.FPSBoost.Enabled then
...
HookIndex = hookmetamethod(game, "__index", function(t, k)
    if t == Mouse and (tostring(k) == "Hit" or tostring(k) == "Target") then
        if IsolatedSettings.SilentAim.Enabled then
            local NearestTarget, NearestPos = IsolatedMiscFunctions.NearestType(IsolatedSettings.Target.UseMouseNearest)
            if NearestTarget and (not IsolatedSettings.SilentAim.DrawingCircle or IsolatedSettings.SilentAim.DrawingCircleSize > NearestPos) and (not IsolatedSettings.Target.Knocked or not IsolatedFunctions.Knocked(NearestTarget)) and (not IsolatedSettings.Target.Grabbed or not IsolatedFunctions.Grabbing(NearestTarget)) and (not IsolatedSettings.Target.WallCheck or (not IsolatedSettings.Target.UseOldWallCheck or (not IsolatedSettings.Target.WallCheck or IsolatedMiscFunctions.WallCheck(NearestTarget.Character.HumanoidRootPart, LocalPlayer.Character.HumanoidRootPart))) and (not IsolatedSettings.Target.UseNewWallCheck or (not IsolatedSettings.Target.WallCheck or IsolatedMiscFunctions.CameraCheck(NearestTarget.Character.HumanoidRootPart.Position, {NearestTarget.Character, LocalPlayer.Character, CurrentCamera}) == true))) and (not IsolatedSettings.SilentAim.UseRange or IsolatedSettings.SilentAim.Range > (LocalPlayer.Character.HumanoidRootPart.Position - NearestTarget.Character.HumanoidRootPart.Position).Magnitude) then
                local TargetBody 
                if NearestTarget.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then 
                    ...
                    TargetBody = NearestTarget.Character[IsolatedSettings.SilentAim.ForceHead]
                end
...
HookNamecall = hookmetamethod(game, "__namecall", function(self, ...)
    ...
    if tostring(self) == "MainEvent" and tostring(Method) == "FireServer" then
        if Args[1] == "CHECKER_1" or Args[1] == "OneMoreTime" then
            if IsolatedSettings.Disabler.Ban then
                AllHookProperty[1] = true
            end
            return
        end
        if Args[1] == "TeleportDetect" then
            if IsolatedSettings.Disabler.CFrameDetection then
    ...
    elseif getfenv(2).crash then
        hookfunction(getfenv(2).crash, function()
            if IsolatedSettings.Disabler.Crash then
...
spawn(function()
    while wait() do
        if AllHookProperty[1] then
            IsolatedFunctions.Notify("Attempted To Ban Bypassed!", 1.5, "Warnning")
            AllHookProperty[1] = false
        end
        if AllHookProperty[2] then
            IsolatedFunctions.Notify("Attempted To CFrame Detection Bypassed!", 1.5, "Warnning")
            AllHookProperty[2] = false
        end
        if AllHookProperty[3] then
            IsolatedFunctions.Notify("Attempted To Crash Bypassed!", 1.5, "Warnning")
            AllHookProperty[3] = false
        end
        if IsolatedSettings.LagDetector.Enabled then
            if math.floor(Stats:GetValue()) > 210 then
                IsolatedFunctions.Notify("Lag Detected, [" .. math.floor(Stats:GetValue()) .. "ms]", 1.5, "Info")
            end
        end
    end
end)

IsolatedFunctions.Notify("Script Loaded", 1.5, "Info")
IsolatedSettings.Loaded = true

spawn(function()
    while wait(0.35) do
        if IsolatedSettings.TargetGui.Enabled then
            local NearestTarget, NearestPos = IsolatedMiscFunctions.NearestType(IsolatedSettings.Target.UseMouseNearest)
            if NearestTarget and IsNetwork(NearestTarget) then
                ...
                HealthText.Text = IsolatedFunctions.Round(NewHealth * 100, 2)
                ...
                if NearestTarget.Character:FindFirstChild("BodyEffects") and NearestTarget.Character:FindFirstChild("BodyEffects"):FindFirstChild("Armor") then
                    ...
                    ArmorText.Text = IsolatedFunctions.Round(NewArmor * 100, 2)
...
while wait(1) do
    IsolatedStorage.GetTime = IsolatedStorage.GetTime + 1
    if IsolatedSettings.Watermark.Enabled then
        local GetHours, GetMinutes, GetSeconds = IsolatedFunctions.CalculateTime(IsolatedStorage.GetTime)
        ...
        Title.Text = "Isolated | Da Hood | " .. IsolatedStorage.FPSBeat .. " fps | " .. tostring(math.floor(Stats:GetValue())) .. " ms | " .. HT .. ":" .. MT .. ":" .. ST
        IsolatedStorage.FPSBeat = 0
